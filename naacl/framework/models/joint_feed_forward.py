...
def define_model(layers, nonlinearity=tf.sigmoid,  weights_sd=0.01, biases=0):
	...
	#input layer 
	activations[0]=tf.nn.dropout(tf.placeholder(tf.float32, shape=[None,layers[0]], name='input_layer'),
							  keep_prob=1.-dropout_embedding)


	#all hidden layers
	for i in np.arange(start=1, stop=len(layers)-1, step=1):
	...
	#output layer
	w_out, b_out, z_out=util.get_layer(	shape=[layers[-2], layers[-1]],  
...
def train(session,
	...
	# batch_gen=util.Batch_Gen(features, labels, batch_size)
...
def predict(features, session, var_names):
	...
	preds=sess.run('output_layer:0',
							 feed_dict={'input_layer:0':features,
									    'dropout_embedding:0':.0,
									    'dropout_hidden:0':.0})
	preds=pd.DataFrame(data=preds, columns=var_names, index=features.index)
	return preds
...
def evaluate(	train_features,
	...
	'''
	Utility method for evaluation the model on given train/test-set
	'''
	results=pd.DataFrame(columns=list(test_labels))
	for i in range(1,runs+1):
		...
		define_model(	layers=layers,
						...
						weights_sd=weights_sd,
						biases=biases)
		define_loss(loss_function=loss_function,
					l2_beta=l2_beta)
		...
		train(session=sess,
			  ...
			  report_at=report_at)
		results.loc[i]=test(	session=sess,
